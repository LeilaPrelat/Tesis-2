...
list_R =  np.linspace(0.5,0.01,99)  
# list_mu = [0.3]
...
if save_data_opt==1:
    ...
    np.savetxt('opt_det_sinkz_vs_R2_modo%i.txt' %(modo), tabla, fmt='%1.9e', delimiter='\t', header = header1)
...
if save_graphs==1:
    os.chdir(path)
    plt.savefig('Im_epsi1_vs_R2_%i'%(modo))
...
if save_graphs==1:
    os.chdir(path)
    plt.savefig('Omegac_vs_R2_%i'%(modo))
