...
for mu in barrido_mu:
    ...
    def Im_omegac_QE(epsi1_imag): #eq 16 del paper 2, aprox cuasi
        rta = omegac_QE(epsi1_imag,modo,re_epsi1,R,mu)
        return rta.imag
...
if save_data==1:
    ...
    tabla = np.transpose(tabla)
    info = '.Opt det sin kz, Re(epsi1)=%.2f, R=%.1f \mum' %(re_epsi1,R) 
    ...
    tabla = np.transpose(tabla)
    info = '.Aprox cuasi sin kz, Re(epsi1)=%.2f, R=%.1f \mum' %(re_epsi1,R) 
    header1 = 'mu [eV]     Im(epsi1)     Im(omega/c)      Re(omega/c)' + info + name_this_py
...
path_load = path_omegacQE + '/real_freq/re_epsi1_%.2f_vs_mu' %(re_epsi1)
os.chdir(path_load)
