...
Calcular Qext como Qscat + Qabs
(formulas que aparece en el overleaf
...
if graph_1D==1:
    ...
    for im_epsi1 in list_im_epsi1_fino:
        ...
        for omeggac in list_omegac:
            ...
            list_Qext1.append(Qscatt + Qabss)  
        list_Qext_tot1.append(list_Qext1)  
    ...
    for im_epsi1 in list_im_epsi1_grueso:
        ...
        for omeggac in list_omegac:
            ...
            Qscatt = Qscat(omeggac,epsi1,nmax,R,hbaramu,Ao)
            Qabss = Qabs(omeggac,epsi1,nmax,R,hbaramu,Ao)
            list_Qext2.append(Qscatt + Qabss)  
        list_Qext_tot2.append(list_Qext2)  
    ...
    for j in range(len(list_Qext_tot1)):
        list_Qext2 = list_Qext_tot1[j]
        ...
        plt.plot(list_omegac,list_Qext2,'o',color = colores[j],ms = 4,alpha = 0.8,label = labell)
    
    n = 10
    mini,maxi = np.min(list_Qext_tot1),np.max(list_Qext_tot1)
    ...
    for j in range(len(list_Qext_tot2)):
        list_Qext2 = list_Qext_tot2[j]
    ...
    n = 10
    mini,maxi = np.min(list_Qext_tot2),np.max(list_Qext_tot2)
    eje_Lambda2 = np.linspace(mini,maxi,n)
    plt.plot(omegac*np.ones(n),eje_Lambda2,'-k',lw = 1,label = labelomegac)
    plt.ylabel(labely,fontsize=tamletra)
    plt.xlabel(labelx,fontsize=tamletra)
...
if graph_2D==1:
    ...
    if zoom==1:
        ...
        tol = 1e-2
        list_im_epsi1 = np.linspace(im_epsi1c - 3*tol,im_epsi1c + 3*tol,N)
