...
for Ep in list_Ep: 
    ...
    for modo in [1,2,3,4]:
        ...
        if modo == 1:
            ...
            axs[2].plot(list_mu_opt,epsi1_imag_opt,symbols[k],lw = lw,color = colors[k],label = 'modo %i'%(k+1))
        else:    
    ...
    for i in [0,1,2]:
        ...
        axs[i].set_ylabel(labely1,fontsize = int(tamletra*1.2),labelpad=labelpady) 
    ...
    for modo in [1,2,3,4]:
        ...
        line1, = plt.plot(list_mu_opt,im_epsi1_QE,symbols[k],lw = lw,color = colors[k],label = 'modo %i'%(k+1))
        if k == 0:
            line1.set_dashes([2, 2, 10, 2])  # 2pt line, 2pt break, 10pt line, 2pt break
    ...
    for modo in [1,2,3,4]:
        ...
        tabla = np.transpose(tabla)
        [list_mu_opt,omegac_opt,epsi1_imag_opt,eq_det] = tabla
        line1, = plt.plot(list_mu_opt,np.array(omegac_opt)*hb*c,symbols[k],lw = lw,color = colors[k],label = 'modo %i'%(k+1))
        if k == 0:
            line1.set_dashes([2, 2, 10, 2])  # 2pt line, 2pt break, 10pt line, 2pt break
            
        k = k + 1
    
    plt.ylabel('Energy [eV]',fontsize=tamletra,labelpad =labelpady)
    plt.xlabel(labelx,fontsize=tamletra,labelpad=labelpadx)
    plt.tick_params(labelsize = tamnum,pad = pad)
    ...
    if save_graphs==1:
        os.chdir(path_load)
        plt.savefig('energy_vs_muR_%.2f.png' %(R),format = 'png')
    ...
    for modo in [1,2,3,4]:
        ...
        line1, = plt.plot(list_mu_opt,omegac_QE,symbols[k],lw = lw,color = colors[k],label = 'modo %i'%(k+1))
        if k == 0:
            line1.set_dashes([2, 2, 10, 2])  # 2pt line, 2pt break, 10pt line, 2pt break
            
        k = k + 1
    
        plt.ylabel(labely2,fontsize=int(tamletra*1.2),labelpad =labelpady)
        plt.xlabel(labelx,fontsize=tamletra,labelpad=labelpadx)
        plt.tick_params(labelsize = tamnum,pad = pad)
    ...
    for modo in [1,2,3,4]:
        ...
        if save_graphs==1:
            os.chdir(path_load)
            plt.savefig('energy_vs_muQE_%.2f.png' %(R),format = 'png')    
