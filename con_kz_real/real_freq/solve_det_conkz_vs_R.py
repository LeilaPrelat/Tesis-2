...
barrido en R para mu = 0.3 y kz fijo.  Hallar omega/c real y im(epsilon1) que minimizan
...
re_epsi1 = 3.9
mu = 0.3             #eV
list_kz = [0,0.0001,0.0005,0.001,0.005,0.01,0.05,0.1,0.13]
list_modos = [1,2,3,4]
    
list_R = np.linspace(0.5,50,5001) 
kzlim = 0.132
...
if save_data_opt==1:
    ...
    path_det = r'/re_epsi1_%.2f_vs_R' %(re_epsi1)
...
def fcond_inicial(kz_real,modo):
    ...
    """
    os.chdir(path_basic + '/' + 're_epsi1_%.2f_vs_kz/mu_%.1f' %(re_epsi1,mu))
...
for kz_real in list_kz:
    for modo in list_modos:
        labeltxt = '_vs_R_kz%.4f_modo%i' %(kz_real,modo)
        cond_inicial = fcond_inicial(kz_real,modo)
        ...
        if save_data_opt==1:
            ...
            info = '.Opt det con kz, mu = %.1f eV, Re(epsi1)=%.2f, kz = %.4f 1/\mum' %(mu,re_epsi1,kz_real) 
            header1 = 'R [1/micrones]     Omega/c [1/micrones]    Im(epsi1)     Eq(det)' + info + ', ' + name_this_py
        ...
        labelx = 'R [1/$\mu$m]'
        labelpng = '_vs_R_kz%.4f_%i' %(kz_real,modo)
        title = 'Modo = %i, $\mu_c$ = %.1f eV, Re($\epsilon_1$) = %.2f' %(modo,mu,re_epsi1) 
        title2 = title + '\n' + name_this_py
        ...
        for j in range(len(list_R_opt)):
            R = list_R_opt[j]
            omegac = omegac_opt[j]
            epsi1_imag = epsi1_imag_opt[j]
            epsi1 = re_epsi1 + 1j*epsi1_imag
            kt_value = kt(kz_real,omegac,epsi1,modo,R,mu)    
        ...
        [barrido_R_sinkz,omegac_opt_sinkz,epsi1_imag_opt_sinkz,eq_det_sinkz] = data_load
        corte = 1000
        
        plt.figure(figsize=tamfig)
        plt.plot(list_R_opt,epsi1_imag_opt,'.-r',ms=10, label = label_graph)
        if kz_real<kzlim:
            plt.plot(barrido_R_sinkz[0:corte],epsi1_imag_opt_sinkz[0:corte],'.-m',ms=10, label = labelsinkz)
        ...
        if kz_real<kzlim:
            plt.plot(barrido_R_sinkz[0:corte],omegac_opt_sinkz[0:corte],'.-m',ms=10, label = labelsinkz)
        plt.title(title2,fontsize=tamtitle)
        ...
        plt.plot(barrido_R_sinkz[0:corte],eq_det_sinkz[0:corte],'.-m',ms=10, label = labelsinkz)
        plt.title(title2,fontsize=tamtitle)
        ...
        plt.figure(figsize=tamfig)
        plt.plot(list_R_opt,kt_abs,'.-r',ms=10,label=label_graph)
        plt.title(title2,fontsize=tamtitle)
        ...
        plt.figure(figsize=tamfig)
        plt.plot(list_R_opt,kt_real,'.-r',ms=10,label=label_graph)
        plt.title(title2,fontsize=tamtitle)
        ...
        plt.grid(1)
        if save_graphs==1:
            plt.savefig('kt_real' + labelpng, format='png')
            
        plt.figure(figsize=tamfig)
        plt.plot(list_R_opt,kt_imag,'.-r',ms=10,label=label_graph)
        plt.title(title2,fontsize=tamtitle)
