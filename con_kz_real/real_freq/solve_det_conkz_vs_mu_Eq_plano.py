...
try:
    ...
    from det_conkz_eq_plano import determinante
...
if save_data_opt==1:
    ...
    np.savetxt('opt_det_conkz_eq_plano' + labeltxt + '.txt', tabla, fmt='%1.11e', delimiter='\t', header = header1)
...
if save_graphs==1:
    os.chdir(path)
    plt.savefig('Im_epsi1_eq_plano' + labelpng + '.png', format='png')
...
plt.grid(1)
if save_graphs==1:
    plt.savefig('Omegac_eq_plano' + labelpng + '.png', format='png')
...
plt.grid(1)
if save_graphs==1:
    plt.savefig('det_eq_plano' + labelpng + '.png', format='png')
# 
#plt.figure(figsize=tamfig)
