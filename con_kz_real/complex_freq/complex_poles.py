...
path_graphene = path_basic.replace('/con_kz/complex_freq','') 
...
list_kz = [0.05,0.08,0.12]

info = 'Modo = %i, R = %.2f $\mu$m, Re($\epsilon_1$) = %.2f' %(modo,R,re_epsi1) +  ', ' + name_this_py

path_g = path_basic + r'/re_epsi1_%.2f_vs_kz' %(re_epsi1)
path_load = path_g  + '/' + 'find_Lambda' + '/' + 'modo_%i/archivos_txt' %(modo)
os.chdir(path_load)
...
for kz in list_kz:
    ...
    try: 
        [epsi1_imag_opt,omega_c_real,omega_c_imag,eq_gn] = tabla
    except ValueError: 
        [epsi1_imag_opt,omega_c_real,omega_c_imag] = tabla
...
fig,ax = plt.subplots(1,len(list_kz), sharey=True, facecolor='w', figsize = tamfig)
...
#    list_im_epsi1.append(epsi1_imag_opt[ind3])
    ...
    ax[i].plot(omega_c_real[0:ind3],omega_c_imag[0:ind3],'-',lw = lw,color = list_color[i],label = 'kz = %.2f $\mu$m$^{-1}$' %(kz))
...
tabla = np.array([list_kz, list_im_epsi1],dtype=object)
tabla = np.transpose(tabla)
