...
for ind in list_ind: 
    ...
    if non_active_medium == 1: #epsi1 = re_epsi1 ---> no hay medio activo
        ...
        if save_graphs==1:
            os.chdir(path_save2)
            plt.savefig('|Etot|2_loss_modo%i_kz%.4f.png' %(modo,kz), format='png')
    ...
    if save_graphs==1:
        os.chdir(path_save2)
        plt.savefig('|Etot|2_modo%i_kz%.4f.png' %(modo,kz), format='png')
    
    del X,Y,Z1,Z2
    if paper == 1:
        np.savetxt('info_Etot_modo%i_kz%.4f.txt' %(modo,kz), [title], fmt='%s')
