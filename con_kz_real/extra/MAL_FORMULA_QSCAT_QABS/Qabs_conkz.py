...
Vamos a usar la formula del Qabs con kz 
...
#%%
#print('Definir el coeficiente Qabs')
...
def Qabs(kz_var,omegac,epsi1,nmax,R,hbaramu,Ao,Bo): 
    ...
    k0 = omegac #=omega/c
    xz = kz_var/k0 #adimensional
    Rbarra = R*k0 #adimensional

    xt2c = -xz**2 + mu2*epsi2 #xt2**2
    inside = xt2c 
    xt2 = (inside)**(1/2)     #xt2  
    if (xt2*Rbarra).real>=0:
	    xt2 = xt2
    else:
	    xt2 = -xt2
    ...
    for nu in list_modos: 
        ...
        Qabs = Qabs - Dn2/epsi2 + Bn2/mu2
     
    ctef = 1/(4*pi*Rbarra)
    Qabsf = Qabs*ctef
    
    return Qabsf #el 1/8pi de la formula de poynting ---> esta dividido por c el resultado
