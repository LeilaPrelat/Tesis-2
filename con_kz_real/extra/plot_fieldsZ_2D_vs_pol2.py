...
autovectores de la matriz del caso INhomogeneo
...
path_save = path_basic + '/' + 'fields_2D_vs_pol2'
...
try:
    ...
    path_basic = input('path de la carpeta donde se encuentra coef_matrixAoBo.py')
    sys.path.insert(1, path_basic)
    from coef_matrix_AoBo import coef
...
coeff = coef(kz,omegac,epsi1,modo,R,hbaramu,Ao,Bo)
...
order1 = math.floor(math.log(np.abs(coef_C1), 10))
order2 = math.floor(math.log(np.abs(coef_A1), 10))
order3 = math.floor(math.log(np.abs(coef_D2), 10))
order4 = math.floor(math.log(np.abs(coef_B2), 10))

exp1,exp2 = 10**order1,10**order2
exp3,exp4 = 10**order3,10**order4

Ao1 = coef_C1/exp1       #Ao es el de Hz ---> coef_D2 de la formula (ver fieldsZ_conkz.py)
Bo1 = coef_A1/exp2
...
Ao2 = coef_D2/exp3       #Ao es el de Hz ---> coef_D2 de la formula (ver fieldsZ_conkz.py)
Bo2 = coef_B2/exp4
...
for rho in list_rho:
    rho = np.abs(rho)
    [Ez1,Ez2,Hz1,Hz2] = fieldsZ(kz,omegac,epsi1,nmax,R,hbaramu,Ao2,Bo2,rho,phi,z)
    if np.abs(rho)<=R:
...
for rho in list_rho:
    ...
    [Ez1,Ez2,Hz1,Hz2] = fieldsZ(kz,omegac,epsi1,nmax,R,hbaramu,Ao,Bo,rho,phi,z)
    if np.abs(rho)<=R:
        Htot_values3.append(np.abs(Hz1))
        Etot_values3.append(np.abs(Ez1))
    else:
        Htot_values3.append(np.abs(Hz2))
        Etot_values3.append(np.abs(Ez2))

#%%

if np.sign(Ao1.imag) == 1 and np.sign(Bo1.imag) == 1:
    labelAoBo = 'Ao = %.3f + i%.3f, Bo = %.3f + i%.3f' %(Ao1.real,Ao1.imag,Bo1.real,Bo1.imag)
...
if np.sign(Ao2.imag) == 1 and np.sign(Bo2.imag) == 1:
    labelAoBo2 = 'Ao = %.3f + i%.3f, Bo = %.3f + i%.3f' %(Ao2.real,Ao2.imag,Bo2.real,Bo2.imag)
elif np.sign(Ao2.imag) == 1 and np.sign(Bo2.imag) == -1:
    labelAoBo2 = 'Ao = %.3f + i%.3f, Bo = %.3f - i%.3f' %(Ao2.real,Ao2.imag,Bo2.real,-Bo2.imag)
elif np.sign(Ao2.imag) == -1 and np.sign(Bo2.imag) == 1:
    labelAoBo2 = 'Ao = %.3f - i%.3f, Bo = %.3f + i%.3f' %(Ao2.real,-Ao2.imag,Bo2.real,Bo2.imag)
else: 
...
plt.plot(list_rho,Htot_values2,'.r',ms=10,label = labelAoBo2)
plt.plot(list_rho,Htot_values3,'.b',ms=10,label = 'Ao = %i, Bo = %i' %(Ao,Bo))
plt.title(title,fontsize=tamtitle)
...
plt.plot(list_rho,Etot_values2,'.r',ms=10,label = labelAoBo2)
plt.plot(list_rho,Etot_values3,'.b',ms=10,label = 'Ao = %i, Bo = %i' %(Ao,Bo))
plt.title(title,fontsize=tamtitle)
