...
for rho in list_rho:
    ...
    if rho <=R:
        Etot_values.append(E1_tot)
    else:
        Etot_values.append(E2_tot)

plt.plot(list_rho,Etot_values,'.',ms=10,label = 'modo = %i' %(modo))
...
if save_graphs==1:
    os.chdir(path_save)
    plt.savefig('|Etot|2_modo%i_index%i.png' %(modo,ind))
...
if save_graphs==1:
    os.chdir(path_save)
    plt.savefig('|Etot|2_loss_modo%i_index%i.png' %(modo,ind))
...
#    info2 = 'nmax = %i, $\mu_c$ = %.1f eV, $\epsilon_1$ = %.1f - i%.2e y $\omega/c$ = %.2e 1/$\mu$m' %(nmax,hbaramu,re_epsi1,-im_epsi1,omegac)
    ...
    if save_graphs==1:
        os.chdir(path_save)
        plt.savefig('|Etot|2_index%i.png' %(ind))
...
if non_active_medium == 1: #epsi1 = re_epsi1 ---> no hay medio activo
    ...
    #    info2 = 'nmax = %i, $\mu_c$ = %.1f eV, $\epsilon_1$ = %.1f - i%.2e y $\omega/c$ = %.2e 1/$\mu$m' %(nmax,hbaramu,re_epsi1,-im_epsi1,omegac)
        ...
        if save_graphs==1:
            os.chdir(path_save)
            plt.savefig('|Etot|2_loss_index%i.png' %(ind))
