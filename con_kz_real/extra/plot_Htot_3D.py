    ...
    for ind in list_ind: 
        ...
        if non_active_medium == 1: #epsi1 = re_epsi1 ---> no hay medio activo
            ...
            def Etot_2variable(x,y):   
                phi = np.arctan2(y,x)
                rho = (x**2+y**2)**(1/2)
                [E1_tot,E2_tot] = Htot(kz,omegac,re_epsi1,nmax,R,hbaramu,Ao,Bo,rho,phi,z)
            ...
            if save_graphs==1:
                os.chdir(path_save2)
                plt.savefig('|Htot|2_loss_modo%i_kz%.4f.png' %(modo,kz), format='png')
        
        def Etot_2variable(x,y):   
            phi = np.arctan2(y,x)
            rho = (x**2+y**2)**(1/2)
            [E1_tot,E2_tot] = Htot(kz,omegac,epsi1,nmax,R,hbaramu,Ao,Bo,rho,phi,z)
            if np.abs(rho) <= R: #medio1
                return E1_tot
            else: #medio2
                return E2_tot
        ...
        if save_graphs==1:
            os.chdir(path_save2)
            plt.savefig('|Htot|2_modo%i_kz%.4f.png' %(modo,kz), format='png')
