...
for ind in list_ind:
    for modo in list_modos:
        ...
        for rho in list_rho:
            ...
            [E10_tot,E20_tot] = Htot(kz,omegac,epsi1,nmax,R,hbaramu,Ao,Bo,rho,phi,z)
            [E11_tot,E21_tot] = Htot(kz,omegac,epsi1,nmax,R,hbaramu,Ao1,Bo1,rho,phi,z)
            [E12_tot,E22_tot] = Htot(kz,omegac,epsi1,nmax,R,hbaramu,Ao2,Bo2,rho,phi,z)
            
            if np.abs(rho)<=R:
                Htot_values0.append(E10_tot)
                Htot_values1.append(E11_tot)
                Htot_values2.append(E12_tot)
            else:
                Htot_values0.append(E20_tot)
                Htot_values1.append(E21_tot)
                Htot_values2.append(E22_tot)
        ...
        # if save_graphs==1:
        #     os.chdir(path_save)
        #     plt.savefig('|Htot|2_index%i_modo%i_bestpol1.png' %(ind,modo))
        ...
        # plt.grid(1)  
        # if save_graphs==1:
        #     plt.savefig('|Htot|2_index%i_modo%i_bestpol2.png' %(ind,modo))
    
        plt.figure(figsize=tamfig)
        plt.plot(list_rho,Htot_values0,'.r',ms=10,label = labelAoBo)
        plt.plot(list_rho,Htot_values1,'.b',ms=11,label = labelAo1Bo1)
        plt.plot(list_rho,Htot_values2,'.m',ms=10,label = labelAo2Bo2)
        plt.title(title,fontsize=tamtitle)
        plt.ylabel(labely,fontsize=tamletra)
        plt.xlabel(labelx,fontsize=tamletra)
        plt.tick_params(labelsize = tamnum)
        ...
        if save_graphs==1:
            os.chdir(path_save)
            plt.savefig('|Htot|2_index%i_modo%i.png' %(ind,modo))
