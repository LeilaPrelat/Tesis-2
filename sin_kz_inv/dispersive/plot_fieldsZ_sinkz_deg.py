...
if modulo==1:
    label = '$|H_z|$'
    print1 = 'Graficar |Hz| para el medio 1 y 2'
else:
    label = 'Re($H_z$)'
    print1 = 'Graficar Re(Hz) para el medio 1 y 2'
if modulo==1:
    label2 = '$|E_z|$'
    print2 = 'Graficar |Ez| para el medio 1 y 2'
else:
    label2 ='Re($E_z$)'
    print2 = 'Graficar Re(Hz) para el medio 1 y 2'
...
list_index1 = [0,1]         #0 ---> saltos de a 1 modo, 1 ---> saltos de a 2 modos
#list_index2 = [0,1,2,3]    #long de cada diccionario
list_index3 = [0,1]         # 1 o 0 tiene que ser (longitud de list_modes)

for index1 in list_index1:
    if index1 == 0:
        list_index2 = [0,1,2,3]
    elif index1 == 1:
        list_index2 = [0,1]
    for index2 in list_index2:
        for index3 in list_index3:     
            
            os.chdir(path_load)
            try:
                np.loadtxt(deg_txt1)
                hbaramu,omegac,delta_ci,list_modes,modo = open_txt(deg_txt1,index1,index2,index3)
                print('deg_txt1 not found')
            except OSError or IOError:
                hbaramu,omegac,delta_ci,list_modes,modo = open_txt(deg_txt2,index1,index2,index3)
                
            
            print('Degeneracion de los modos:',list_modes,'cerca del modo ', modo)
            print('Im(epsilon1): ', delta_ci, 'mu:', hbaramu, 'omega/c:', omegac)
