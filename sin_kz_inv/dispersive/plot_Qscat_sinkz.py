...
path_sinkz = path_basic.replace('/sin_kz_inv/dispersive','')
path_sinkz = path_sinkz + '/sin_kz/dispersive/real_freq'
...
delta_ci_inv = epsi1_imag_opt_inv[index]
...
if zoom == 0:
    ...
    path_sinkz = path_sinkz + '/R_%.2f/epsiinf_DL_%.2f_vs_mu/Ep_%.1f' %(R,epsiinf_DL,Ep)
    os.chdir(path_sinkz)
...
info1 = 'R = %.2f $\mu$m, $\mu_c$ inv = %.4f eV, $\epsilon_\infty$ = %.1f, $\gamma_{DL}$ = %.2f eV, Ep = %.1f eV' %(R,hbaramu_inv,epsiinf_DL,gamma_DL,Ep)
