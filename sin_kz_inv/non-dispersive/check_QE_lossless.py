...
for modo in list_modes:
    ...
    for mu in list_mu:
        ...
        def im_omegac_QE(x):
            rta = omegac_QE_inv(x,modo,re_epsi1,R,mu)
            return rta.imag
        ...
        omegac = omegac_QE_inv(sol,modo,re_epsi1,R,mu)
    ...
    if save_data_opt==1:
        ...
        np.savetxt('opt_QE_inv_vs_mu_modo%i.txt' %(modo), tabla, fmt='%1.11e', delimiter='\t', header = header1)
