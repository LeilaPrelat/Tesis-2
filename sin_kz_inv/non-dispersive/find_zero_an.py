...
(an es el coef de Hz2, medio exterior del cilindro, 
 an tiene como denominador -gn, minimizamos gn para 
 el caso del polo)
...
def determinante(omegac,epsi1,mode,R,mu_c):
    ...
    en lugar de minimizar el denominador de los coef
    an y cn (caso sin kz no dispersivo) al que llamamos
    gn, ahora vamos a minimizar al numerador de -an porque
    su denominador es -gn y se tachan los menos
    (an es el coef de Hz2, medio exterior del cilindro)
