...
graph_2D = 0 #graficar 3D mapa de color el coef an y el modelo para el coef an (graficos de color)
log_scale = 0
...
def list_omega(typpe,modo,mu):
    """
    Parameters
    ----------
    typpe : resonancia (res) o invisibilidad (inv)
    modo : modo entero
    mu : potencial quimico del grafeno en eV
    Raises
    ------
    TypeError
        hay solo 2 tipos posibles: resonancia e invisibilidad
    
    Returns
    -------
    list_omegac_real : lista omega/c real valores criticos
    list_omegac_imag : lista omega/ imaginario valores criticos
    list_epsi1_imag : lista Im(epsilon1) valores criticos 
    ...
    return list_omegac_real,list_omegac_imag,list_epsi1_imag

#%%

def list_omega_around(typpe,modo,mu,index,tol,N):
    """
    Parameters
    ----------
    typpe : resonancia (res) o invisibilidad (inv)
    modo : modo entero
    mu : potencial quimico del grafeno en eV
    index : indice del valor critico alrededor del cual queremos graficar
    tol : qu'e tan cerca del valor critico queremos graficar
    ...
    list_omegac_imag : lista omega/ imaginario valores criticos
    list_epsi1_imag : lista Im(epsilon1) valores criticos
    omegac_real : Re(omega/c) critico alrededor del cual graficar 
    omegac_imag : Im(omega/c) critico alrededor del cual graficar 

    """  
         
    list_omegac_real,list_omegac_imag,list_epsi1_imag = list_omega(typpe,modo,mu)
...
listx,listy,list_epsi1_imag, omegac_real,omegac_imag = list_omega_around('inv',modo,mu,index,tol,N)
omegac_inv = omegac_real + 1j*omegac_imag 

listx,listy,list_epsi1_imag, omegac_real,omegac_imag = list_omega_around('res',modo,mu,index,tol,N)
...
if graph_2D == 1:
    ...
    if save_graphs==1:
        ...
        plt.savefig('coef2D_an%i'%(modo))
    
    print('Grafico mapa de color del MODELO del coeficiente an cerca de los valores criticos')
    ...
    if save_graphs==1:
        #plt.tight_layout(pad=wspace)
        plt.savefig('modelo2D_coef_an%i'%(modo))
...
list_epsi1_imag_tot = []
list_cte_real_crit_tot = []
list_cte_imag_crit_tot = []

for modo in [1,2,3,4]:
    list_cte_real_crit = []
    list_cte_imag_crit = []
    
    list_omegac_real,list_omegac_imag,list_epsi1_imag = list_omega('res',modo,mu)
    ...
    for index in range(len(list_epsi1_imag)):
        ...
        list_cte_real_crit.append(float(cte_real_crit))
        list_cte_imag_crit.append(float(cte_imag_crit))
    ...
    list_cte_real_crit_tot.append(list_cte_real_crit)
    list_cte_imag_crit_tot.append(list_cte_imag_crit)
...
plt.tick_params(labelsize = tamnum)
for i in range(4):
    plt.plot(list_epsi1_imag_tot[i],np.abs(list_cte_real_crit_tot[i]),'.',ms = 10,label = 'modo = %i' %(i+1))
plt.yscale('log')
plt.legend(loc='best',markerscale=2,fontsize = tamlegend )
plt.grid(1)
...
plt.tick_params(labelsize = tamnum)
for i in range(4):
    plt.plot(list_epsi1_imag_tot[i],np.abs(list_cte_imag_crit_tot[i]),'.',ms = 10,label = 'modo = %i' %(i+1))
plt.yscale('log')
plt.legend(loc='best',markerscale=2,fontsize = tamlegend )
plt.grid(1)
